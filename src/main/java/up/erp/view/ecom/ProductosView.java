...
public class ProductosView extends ProductosUI {
    ...
    public void On_find_LineaEcom() {
        ...
        for (int i = 0; i < result.size(); i++) {
            ...
            if (StockPorLlegar.getCantidad_por_llegar() == null) {
                if (stock_real >= stock_paraVenta)
                    stock_disponible = stock_real - stock_paraVenta;
                else
                    stock_disponible = 0;
            } else {
                stock_added = stock_real + StockPorLlegar.getCantidad_por_llegar();
                if (stock_added >= stock_paraVenta) {
                    stock_disponible = stock_added - stock_paraVenta;
                } else
                    stock_disponible = 0;
    ...
    public void on_Click_ComboLinea() {
         ...
         * list_Linea_ecom.get(1).getNombre(), list_Linea_ecom.get(2).getNombre(),
         * list_Linea_ecom.get(3).getNombre(), list_Linea_ecom.get(4).getNombre());
         */
    }
    ...
    public void on_Open_Ecomdialog() {
        ...
        if (list_aux.size() == 1 && !list_aux.isEmpty()) {
            ...
            if (prod_add.getEcommerce() == null) {
                checkEcom.setValue(Boolean.FALSE);
            } else {
                checkEcom.setValue(prod_add.getEcommerce());
            }
            if (prod_add.getPromocion() == null) {
                checkPromo.setValue(Boolean.FALSE);
            } else {
                checkPromo.setValue(prod_add.getPromocion());
            }
            if (prod_add.getNuevo() == null) {
                checkNuevo.setValue(Boolean.FALSE);
            } else {
                checkNuevo.setValue(prod_add.getNuevo());
            }

            if (prod_add.getEcommerce_nombre() != null) {
                txtNameEcom.setValue(prod_add.getEcommerce_nombre());
            } else {
                txtNameEcom.setValue("");
            }

            if (prod_add.getEcommerce_descrip() != null) {
                txtaDesc_ecom.setValue(prod_add.getEcommerce_descrip());
            } else {
                txtaDesc_ecom.setValue("");
            }

            if (prod_add.getEcom_precio() != null) {
                txtPrecioEcom.setValue(String.valueOf(prod_add.getEcom_precio()));
            } else {
                txtPrecioEcom.setValue("");
            }
    ...
    public void on_Grabar_ecom() {
        ...
        if (!txtUpload1.isEmpty()) {
            prod_ecom.setEcom_img1_nombre(Url_img1);
        } else {
            prod_ecom.setEcom_img1_nombre(img_url1);
        }
        // Ecommerce imagen 2
        if (!txtUpload2.isEmpty()) {
            prod_ecom.setEcom_img2_nombre(Url_img2);
        } else {
            prod_ecom.setEcom_img2_nombre(img_url2);
        }
        // Ecommerce imagen 3
        if (!txtUpload3.isEmpty()) {
            prod_ecom.setEcom_img3_nombre(Url_img3);
        } else {
            prod_ecom.setEcom_img3_nombre(img_url3);
        }
    ...
    public void init_ReporterToolbar() {
        ...
        for (int i = 0; i < L_gridContent_size; i++) {
            codigo = "";
            if (result.get(i).getCodigo() != null) {
                codigo = result.get(i).getCodigo();
            }
            System.out.println("Codigo: " + codigo);
            Toolbar.ROWS[i].add(codigo);

            nombre = "";
            if (result.get(i).getNombre() != null) {
            ...
            descrip_ecom = "";
            if (result.get(i).getDescripcion() != null) {
                descrip_ecom = result.get(i).getDescripcion();
            }
            System.out.println("Descripcion Ecommerce: " + descrip_ecom);
            Toolbar.ROWS[i].add(descrip_ecom);

            stock = 0;
            if (result.get(i).getStock_ecom() != null) {
                stock = result.get(i).getStock_ecom();
            }
    ...
    // --------------------------------------------------------------------------
    @Override
    public void onOpenCaracteristicaDialog() {
        dialogCaracteristica.open();
    }
